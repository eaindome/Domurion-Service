  ...
  <div class="grid grid-cols-6 gap-4 items-center">
    ...
    {#if showDetails}
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <!-- Modal Content -->
        <div class="relative bg-white rounded-3xl shadow-2xl max-w-lg w-full animate-modal-enter border border-gray-100">
          <!-- Header -->
          <div class="relative px-8 pt-8 pb-6 border-b border-gray-100">
            <button
              class="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              on:click={closeModal}
            ...
            <div class="flex flex-col items-center text-center">
              <div class="relative mb-4">
                <img
                  src={getSiteFavicon(item.siteUrl)}
                  alt="{item.siteName} favicon"
                  class="w-16 h-16 rounded-2xl border border-gray-200 shadow-md"
                  on:error={(e) => {
                    const target = e.target as HTMLImageElement | null;
                    if (target) {
              ...
              <h2 class="text-2xl font-bold text-gray-900 mb-2">{item.siteName}</h2>
              <a 
                href={item.siteUrl} 
                target="_blank" 
          ...
          <div class="px-8 py-6 space-y-6">
            ...
            <div class="group">
              <div class="flex items-center justify-between mb-2">
                <label for="username-field" class="text-sm font-medium text-gray-700">Username</label>
                <button
                  on:click={() => copyToClipboard(item.username, 'username')}
                  class="flex items-center px-2 py-1 text-xs text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-all duration-200"
                >
              ...
              <div class="relative">
                <input
                  ...
                  value={item.username}
                  readonly
                  class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
                  on:click={() => copyToClipboard(item.username, 'username')}
                />
              </div>
            </div>
            ...
            <div class="group">
              <div class="flex items-center justify-between mb-2">
                <label for="password-field" class="text-sm font-medium text-gray-700">Password</label>
                <div class="flex items-center space-x-2">
                  <!-- Password Strength Indicator -->
                  <div class="flex items-center">
                    <div class="w-2 h-2 rounded-full mr-1 bg-{passwordStrength.color}-500"></div>
              ...
              <div class="relative">
                <input
                  ...
                  value={item.password}
                  readonly
                  class="w-full px-4 py-3 pr-12 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
                  on:click={() => copyToClipboard(item.password, 'password')}
                />
                <button
            ...
            {#if item.notes}
              <div class="group">
                ...
                <textarea
                  ...
                  value={item.notes}
                  readonly
                  class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
                  rows="3"
                  on:click={() => copyToClipboard(item.notes ?? '', 'notes')}
                ></textarea>
              </div>
            ...
            <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
              <div class="text-center">
                <div class="text-xs font-medium text-gray-500 mb-1">Created</div>
          ...
          <!-- Actions Footer -->
          <div class="px-8 py-6 bg-gray-50 rounded-b-3xl border-t border-gray-100">
            <div class="flex items-center justify-between">
              <button
                on:click={handleEdit}
                class="flex items-center px-4 py-2 text-sm text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors"
              >
              ...
              <button
                on:click={handleDelete}
                class="flex items-center px-4 py-2 text-sm text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors"
              >
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
