...
namespace Domurion.Services.Interfaces
{
    public interface IPasswordVaultService
    {
        ...
        Credential UpdateCredential(Guid credentialId, Guid userId, string? site, string? username, string? password);
        void DeleteCredential(Guid credentialId, Guid userId);
